/* $Id: spp_frag3.c,v 1.22.2.7 2006/03/24 17:33:17 ssturges Exp $ */
...
/* the twiddle is needed for architectures that force word-aligned memory
 * access 
 */
...
static Packet *defrag_pkt;               /* holder for prealloc'd defrag pkt */
        ...
        Frag3Context *f3context)
{
    ...
    char lastfrag = 0;       /* Set to 1 when this is the 'last' frag */
            ...
            switch(f3context->frag_policy)
            {
                ...
                case FRAG_POLICY_FIRST:
...

                    {
                        ...
                        {
                            /* Adjust offset to end of 'right' */
                            frag_offset = right->offset + right->size;
                            /* Not overlapping to the left anymore */
                            slide = 0;
                            /* Set 'left' so the next insert goes
                             * after the current 'right'
                             */
                            left = right;
                            /*
...
static void Frag3InitPkt()
{
     ...
     */
    defrag_pkt->pkth =  SnortAlloc(sizeof(SnortPktHeader));
    defrag_pkt->pkt = (u_int8_t *) SnortAlloc(DATASIZE + SPARC_TWIDDLE);
    ...
    {
        FatalError("Out of memory on Frag3InitPkt()\n");
    }
}
    ...
    else
    {
        ...
        while(prealloc_nodes_in_use>(global_config.static_frags-ten_percent))
        {
            ...
            if (hnode && hnode->data == not_me)
            {
                ...
                {
                    /* Uh, problem... we've gone through the entire list */
                    LogMessage("(spp_frag3) Frag3Prune: Pruning by prealloc - empty list! ");
                    return pruned;
                }
                sfxhash_gmovetofront(f_cache, hnode);
                found_this = 1;
