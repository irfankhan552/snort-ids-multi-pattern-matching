/* $Id: spp_stream4.c,v 1.184.2.10 2006/05/01 21:06:53 ssturges Exp $ */
...
StreamAPI s4api = {
    ...
    Stream4DropTraffic, /* Not supported in Stream4 */
    Stream4DropPacket,
...
{
    OverlapData *overlap_info = (OverlapData *)data;
    StreamPacketData *iStream; 
    iStream = ((StreamPacketData *)ItemPtr);

    if ((iStream->seq_num > overlap_info->seq_low) &&
        (iStream->seq_num < overlap_info->seq_hi))
        return 1;
...
void DisplayStream4Config(void) 
{
    ...
    if (s4data.cache_clean_percent != 0)
        LogMessage("    Session cleanup percentage: %f %%\n", s4data.cache_clean_percent);
    else
...
void ParseStream4Args(char *args)
{
    ...
    while(i < num_toks)
    {
        ...
        else if(!strcasecmp(stoks[0], "cache_clean_percent"))
        {
            if(isdigit((int)stoks[1][0]))
            {
                s4data.cache_clean_percent = atof(stoks[1]);

                if ((s4data.cache_clean_percent < 0) ||
                    (s4data.cache_clean_percent > 5))
                {
                    LogMessage("WARNING %s(%d) => Ludicrous (%f) cache cleanup "
                               "percentage, setting to default (%f %%)\n", 
                               file_name, file_line, STREAM4_CACHE_PERCENT);
                    
                    s4data.cache_clean_percent = STREAM4_CACHE_PERCENT;
                }
            }
            else
            {
                FatalError("%s(%d) => Bad cache cleanup percent in "
                           "config file, %d\n", file_name, file_line);
...
void FlushDeletedStream(Session *ssn, Stream *s)
{
    ...
    {
#ifdef PKT_STORE_SPLAY_TREE
        if (ubi_trCount((ubi_btNodePtr)&s->data))
        {
            /* Grab the last packet that was stored to use as the 'reason
             * for flushing'. */
            ubi_btRootPtr RootPtr = &s->data;
            DEBUG_WRAP(DebugMessage(DEBUG_STREAM,
                "Dropping session... reassembling before purge\n"););
            spd = (StreamPacketData *)ubi_trLast(RootPtr->root);
...
void StoreStreamPkt2(Session *ssn, Packet *p, u_int32_t pkt_seq)
{
    ...
    /* Packet will be fast-tracked if list is empty */
...
void StoreStreamPkt(Session *ssn, Packet *p, u_int32_t pkt_seq)
{
    ...
    if(returned != NULL)
    {
        ...
        if(returned->payload_size == p->dsize)
        {
            ...
            if(s->last_ack < pkt_seq + p->dsize)
            {
                ...
                if(returned->cksum != p->tcph->th_sum)
                {
                    ...
                    if (s4data.reassy_method == METHOD_FAVOR_NEW)
                    {
                        /* Remove the old guy from the tree */
                        foo = (StreamPacketData *) ubi_sptRemove(&s->data, 
                        ...
                        /* New packet will be inserted below */
                    }
                    else
                    {
                        /* keep the old data, free up the mem we just alloc'd */
        ...
        else if(returned->payload_size < p->dsize)
        {
            ...
            {
                DEBUG_WRAP(DebugMessage(DEBUG_STREAM,
                            "Adding un-ack'd duplicate packet with forward overlap\n"););
...
#endif
                DEBUG_WRAP(DebugMessage(DEBUG_STREAM,
                            "Adding un-ack'd duplicate packet with backwards overlap\n"););
                s->overlap_pkts++;
    ...
    {
        /* Nothing with that sequence number... Check if there is
         * an overlapping node already in the tree. */
        OverlapData overlapData;
        overlapData.seq_low = pkt_seq;
        overlapData.seq_hi = pkt_seq + spd->pkt_size;
        if (ubi_trCheck(&s->data, OverlapCompareFunc, (void *)&overlapData))
        {
            DEBUG_WRAP(DebugMessage(DEBUG_STREAM,
                            "Adding non-duplicate packet with overlap\n"););
            s->overlap_pkts++;
        }
    }
    ...
    if(ubi_sptInsert(&s->data,(ubi_btNodePtr)spd,(ubi_btNodePtr)spd, &oldNode)
       == ubi_trFALSE)
    {
        LogMessage("sptInsert failed, that sucks\n");

        /* Might as well free up the memory that we couldn't insert */
        stream4_memory_usage -= spd->pkt_size;
        free(spd->pktOrig);
...
#ifdef PKT_STORE_SPLAY_TREE
        if (ubi_trCount(&stream->data) != count)
#else
...
int LogStream(Stream *s)
{
    ...
    if((pv.log_bitmap & LOG_TCPDUMP) && s4data.log_flushed_streams)
    {
        ...
        if (s4data.reassy_method == METHOD_FAVOR_OLD)
        {
            (void)ubi_trTraverse(&s->data, LogTraverse, s);
        }
        else
        {
            (void)ubi_trTraverseReverse(&s->data, LogTraverse, s);
        }
#else
...
int BuildPacket(Stream *s, u_int32_t stream_size, Packet *p, int direction)
{
    ...
    {
        (void)ubi_trTraverseReverse(&s->data, TraverseFunc, &bd);
    }
#else
    {
        StreamPacketData *spd = s->seglist;
        ...
        /* See if there was only 1 packet in the rebuilt.   
         * This means head of list is used, but next seqment wasn't.
         *
         * If so, unset bd.total size, mark the first packet as unused,
         * and return -1.
         */
        if (s->seglist && s->seglist->next &&
            s->seglist->chuck != 0 && /* First one was used */
            s->seglist->next->chuck == 0)  /* Next one wasn't */
        {
            stream_pkt->dsize = 0;
            s->seglist->chuck = 0;
        ...
        stream_pkt->tcph->th_seq = htonl(s->seglist->seq_num);
    }
#endif
...
#ifdef PKT_STORE_SPLAY_TREE
static void TraverseReassembly(ubi_trNodePtr NodePtr, void *foo)
#else
                                    ...
                                    void *userdata)
{
    ...
    if (p)
    {
        ...
        if (s4data.reassy_method == METHOD_FAVOR_OLD)
        {
            (void)ubi_trTraverse(&s->data, TraverseReassembly, &callbackData);
        }
        else
        {
            (void)ubi_trTraverseReverse(&s->data, TraverseReassembly, &callbackData);
        }
#else
        {
            StreamPacketData *spd = s->seglist;
    ...
    int packets = 0;
             ...
             spd = (StreamPacketData *)ubi_btNext((ubi_btNodePtr)spd))
        {
            ...
            {
                packets++;
                if(callback(&spd->pkth, spd->pkt, userdata))
                    break;
            }
        }
    }
