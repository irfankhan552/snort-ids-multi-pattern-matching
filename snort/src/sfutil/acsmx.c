...
*
*/ 
static acsm_summary_t summary={0,0,0}; 
...
AddPatternStates (ACSM_STRUCT * acsm, ACSM_PATTERN * p) 
{
  ...
  pattern = p->patrn;
    ...
    for (; n > 0; pattern++, n--)
    {
      ...
      acsm->acsmStateTable[state].depth = pattern - p->patrn + 1;/*ADDED THIS 04/2006*/
    }
		...
		int offset, int depth, void * id, int iid) 
{
  ...
  if(p->minLen == 0 || p->minLen > n) {
    p->minLen = n; // keep track of the length of the shortest pattern
  }
