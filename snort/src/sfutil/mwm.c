/*
**
** $Id: mwm.c,v 1.6 2006/01/09 20:33:38 ssturges Exp $
**
...
**
  ...
  NOTES:
   ...
   07/08/05 - Steven Sturges -
               Fixed issue when a case-sensitive pattern was missed when
               a incorrect case instance occurs before the correct case
               instance.
*/
  ...
  */
  for(i=0;i<ps->msNumPatterns;i++)
  {
    if( ps->msPatArray[i].psLen > 1 )
    {
       ...
       ningroup = 1;  
       while( (++i < ps->msNumPatterns) && (ps->msPatArray[i].psLen > 1) && (hindex==HASH16(ps->msPatArray[i].psPat)) )
         ningroup++;
