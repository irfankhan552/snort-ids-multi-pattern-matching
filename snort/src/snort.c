/* $Id: snort.c,v 1.239.2.10 2006/03/24 17:32:18 ssturges Exp $ */
/*
...
static void LoadDynamicPlugins()
{
    ...
    {
        LoadDynamicEngineLib(pv.dynamicEngine[i], 0);
    }
...
int s_packetBitOpInit = 0;
static BITOP packetBitOp;
static BITOP fragBitOp; // need another one for frag packets -- don't want to
                        // reuse the bitop from the original packet here.
...
{
    Packet p;
    if (!s_packetBitOpInit)
    {
        boInitBITOP(&packetBitOp, num_preprocs + 1);
        boInitBITOP(&fragBitOp, num_preprocs + 1);
        s_packetBitOpInit = 1;
    }
...
int ShowUsage(char *progname)
{
    ...
    FPUTS_BOTH ("        -B <mask>  Obvuscated IP addresses in alerts and packet dumps using CIDR mask\n");
...
#endif
    FPUTS_UNIX ("   --pid-path <path>               Specify the path for Snort's PID file\n");
...
void ParseDynamicLibInfo(int type)
{
    ...
    switch (type)
    {
        ...
        case DYNAMIC_LIBRARY_DIRECTORY:
            ...
            break;
        case DYNAMIC_ENGINE: /* Load dynamic engine lib specified */
            ...
            pv.dynamicEngine[pv.dynamicEngineCount] = strdup(optarg);
...
#ifdef DYNAMIC_PLUGIN
   {"dynamic-engine-lib", LONGOPT_ARG_REQUIRED, NULL, DYNAMIC_ENGINE},
...
int ParseCmdLine(int argc, char *argv[])
{
    ...
    */
    ...
    while((ch = getopt_long(argc, argv, valid_options, long_options, &option_index)) != -1)
    {
        ...
        {
#ifdef DYNAMIC_PLUGIN
            case DYNAMIC_ENGINE: /* Load dynamic engine specified */
