/* $Id: pp_ftp.c,v 1.1.2.2 2006/03/29 19:06:45 ssturges Exp $ */
/*
...
/*
 * Function: adjust_dir(FTP_SESSION *Session, int cmd,
 *                      char *result, int resultLen)
 *
 * Purpose: Updates the state information that tracks the FTP
 *          directory.
 * 
 * Arguments: Session        => Pointer to session info
 *            cmd            => CDUP, CWD, or PWD
 *            result         => Pointer to response for PWD
 *            resultLen      => Length of above
 *
 * Returns: void => None
 *
 */
void adjust_dir(FTP_SESSION *Session, int cmd, char *result, int resultLen)
{
    char *dirPtr;
    int iLen;
    ...
    switch (cmd)
    {
    case CDUP_CMD_ISSUED:
        if (Session->curr_directory)
        {
            FTP_DIR_NODE *old = Session->curr_directory;
            Session->curr_directory = Session->curr_directory->parent;
            FTPFreeDirectory(old);
            if (Session->curr_directory)
                Session->curr_directory->next = NULL;
            else
                Session->head_directory = NULL;
        }
        else
        {
            /* CD beyond root... problem */
            /* TODO: set an alert here... ? */
            Session->dir_state = LOST_STATE;
        }
        break;
    case CWD_CMD_ISSUED:
        if (Session->dir_adjust[0] == '/')
        {
            FTPFreeDirectory(Session->head_directory);
            Session->head_directory = Session->curr_directory = NULL;
        }
        else if (Session->dir_adjust[0] == '~')
        {
            /* Can't really guess at the home dir for user x... */
            Session->dir_state = LOST_STATE;
        }

        dirPtr = strtok(Session->dir_adjust, "/");
        while (dirPtr)
        {
            FTP_DIR_NODE *thisNode;

            /* Trying to include . as a directory.
             * Nothing to do... */
            if (!strcmp(dirPtr, "."))
            {
                dirPtr = strtok(NULL, "/");
            ...
            iLen = strlen(dirPtr);
            thisNode = malloc(sizeof(FTP_DIR_NODE));
            thisNode->name = calloc(sizeof(char), iLen + 1);
            memcpy(thisNode->name, dirPtr, iLen);

            if (!Session->curr_directory)
            {
                Session->curr_directory = thisNode;
                Session->head_directory = thisNode;
                thisNode->parent = NULL;
                thisNode->next = NULL;
            }
            else
            {
                thisNode->parent = Session->curr_directory;
                thisNode->next = NULL;
                Session->curr_directory->next = thisNode;
                Session->curr_directory = thisNode;
            }
            dirPtr = strtok(NULL, "/");
        }
        break;
    case PWD_CMD_ISSUED:
        /* This is a special case -- we can check state and/or
         * reset, as necessary.  Start at head, check.  If
         * we find a mismatch, update current dir.  :)
         */
        {
            char normalizedDir[1024];
            int i = 0 ,j = 0;
            int quotes_seen = 0;
            FTP_DIR_NODE *thisNode = Session->head_directory;
            int reset = 0;
            memset(&normalizedDir[0], 0, 1024);
            /* First normalize the directory name from the response
             * to eliminate the escaped " -- noted by a "".  The
             * directory name is encapsulated in quotes.
             */          
            for (i=0;i<resultLen && quotes_seen < 2 && j < 1024; )
            {
                if (result[i] == '\"')
                {
                    if ((i < resultLen -1) && (result[i+1] == '\"'))
                    {
                        normalizedDir[j++] = result[i++];
                        i++; /* Skip the 2nd " */
                    }
                    else
                    {
            ...
            /* Now search for each dir component in turn */
            dirPtr = strtok(normalizedDir, "/");
            while (dirPtr && !reset)
            {
                if (!thisNode)
                {
                ...
                if (strcmp(thisNode->name, dirPtr))
                {
                    /* Uh, lost state, kill everything below here */
                    FTPFreeDirectory(thisNode->next);
                    free(thisNode->name);
                    iLen = strlen(dirPtr);
                    thisNode->name = calloc(sizeof(char), iLen + 1);
                    memcpy(thisNode->name, dirPtr, iLen);
            ...
            /* If reset == 1, there was a mismatch of what we
             * had before.  Update it now */
            while (dirPtr && reset)
            {
                iLen = strlen(dirPtr);
                thisNode = malloc(sizeof(FTP_DIR_NODE));
                       ...
                       FTP_CLIENT_REQ *req, int rsp_code)
{
    ...
    switch (Session->user_state)
    {
    case USER_CMD_ISSUED:
        if (rsp_code == 331)
            Session->user_state = PASS_REQUIRED;
        else if (rsp_code == 230)
            Session->user_state = USER_LOGGED_IN;
        else
        {
            Session->user_state = NO_STATE;
            if (Session->user)
            {
                free(Session->user);
                Session->user = NULL;
...
#ifdef MAINTAIN_DIR_STATE
    switch(Session->dir_state)
    {
    case PWD_CMD_ISSUED:
        if (rsp_code == Session->expected_dir_response)
        {
            adjust_dir(Session, PWD_CMD_ISSUED,
                req->param_begin, req->param_size);
        }
        Session->expected_dir_response = 0;
        Session->dir_state = NO_STATE;
        break;
    case CWD_CMD_ISSUED:
        if (rsp_code == Session->expected_dir_response)
        {
            adjust_dir(Session, CWD_CMD_ISSUED, NULL, 0);
        }
        free(Session->dir_adjust);
        Session->dir_adjust = NULL;
        Session->expected_dir_response = 0;
        Session->dir_state = NO_STATE;
        break;
    case CDUP_CMD_ISSUED:
        if (rsp_code == Session->expected_dir_response)
        {
            adjust_dir(Session, CDUP_CMD_ISSUED, NULL, 0);
        }
        Session->expected_dir_response = 0;
        Session->dir_state = NO_STATE;
        break;
    ...
    while (req->pipeline_req)
    {
        ...
        default:
            ...
            if (CmdConf)
            {
                ...
                {
                    if ((req->param_begin[0] == 'T') ||
                        (req->param_begin[0] == 't'))
                        ftpssn->encr_state = AUTH_TLS_CMD_ISSUED;
                    else if ((req->param_begin[0] == 'S') ||
                        (req->param_begin[0] == 's'))
...
#ifdef MAINTAIN_USER_STATE
                if (CmdConf->login_cmd)
                {
                    if (ftpssn->user_state == NO_STATE)
                    {
                        ftpssn->user_state = USER_CMD_ISSUED;
                        /* Save username -- malloc/memcpy */
                        ftpssn->user = calloc(sizeof(char),
                                               req->param_size+1);
                        memcpy(ftpssn->user, req->param_begin,
                            req->param_size);
                    }
                    else if (ftpssn->user_state == PASS_REQUIRED)
                    {
                        ftpssn->user_state = PASS_CMD_ISSUED;
                    }
                }
#endif

#ifdef MAINTAIN_DIR_STATE
                if (CmdConf->dir_response)
                {
                    if ((req->cmd_begin[0] == 'P') ||
                        (req->cmd_begin[0] == 'p'))
                    {
                        ftpssn->dir_state = PWD_CMD_ISSUED;
                        ftpssn->expected_dir_response = CmdConf->dir_response;
                    }
                    else if (ftpssn->dir_state != LOST_STATE)
                    {
                        if ((req->cmd_begin[1] == 'W') ||
                            (req->cmd_begin[1] == 'w'))
                        {
                            ftpssn->dir_state = CWD_CMD_ISSUED;
                            ftpssn->expected_dir_response =
                                CmdConf->dir_response;
                            /* Save directory adjustment -- malloc/memcpy */
                            ftpssn->dir_adjust = calloc(sizeof(char),
                                                         req->param_size+1);
                            memcpy(ftpssn->dir_adjust, req->param_begin,
                                   req->param_size);
                        }
                        else
                        {
                            ftpssn->dir_state = CDUP_CMD_ISSUED;
                            ftpssn->expected_dir_response =
                                CmdConf->dir_response;
                        }
                    }
