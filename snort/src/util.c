/* $Id: util.c,v 1.64.2.8 2006/04/27 20:52:57 ssturges Exp $ */
/*
        ...
        else
        {
            ...
            else
            {
                ...
                LogMessage("    Analyzed: %u(%.3f%%)\n", pc.total,
                        ps.ps_recv?CalcPct((float)(pc.total), 
                        (float) ps.ps_recv):0);
                LogMessage("    Dropped: %u(%.3f%%)\n", ps.ps_drop, 
                        ps.ps_recv?CalcPct((float)ps.ps_drop, (float) ps.ps_recv):0);
                LogMessage("    Outstanding: %u(%.3f%%)\n", ps.ps_recv - ps.ps_drop - pc.total,
                        ps.ps_recv?CalcPct((float)(ps.ps_recv-ps.ps_drop - pc.total), 
                        (float) ps.ps_recv):0);
            }
        }
    }
...
{
#ifndef WIN32
    kill(getppid(), SIGNAL_SNORT_CHILD_READY);
