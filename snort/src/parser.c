/* $Id: parser.c,v 1.109.2.7 2006/05/01 21:06:52 ssturges Exp $ */
/*
...
void ParseDynamicEngine(char *args)
{
    ...
    if (num_toks != 2)
    {
        FatalError("%s(%d) => Missin/incorrect dynamic engine specifier.\n", 
                    file_name, file_line);
    }
    if((pv.dynamicEngine[pv.dynamicEngineCount] = calloc(1, strlen(toks[1]) + 1)) == NULL)
        FatalPrintError("malloc");

    bcopy(toks[1], pv.dynamicEngine[pv.dynamicEngineCount], strlen(toks[1]));
...
void ParseDynamicDetection(char *args)
{
    ...
    {
        FatalError("%s(%d) => Missin/incorrect dynamic detection lib "
                    "specifier.\n", 
                    file_name, file_line);
    }
