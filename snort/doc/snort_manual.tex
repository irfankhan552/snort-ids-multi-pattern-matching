% $Id: snort_manual.tex,v 1.49.2.8 2006/04/27 20:54:14 ssturges Exp $
% 
...
\title{Snort\texttrademark  Users Manual\\  2.6.0RC1 }
...
As of Snort 2.6.0, the commandline flags \texttt{--alert-before-pass}
...
the log directory.  In Snort 2.6, the \texttt{--pid-path} commandline
...
In Snort v2.4, a \texttt{-G} command line option was added that specifies
an instance identifier for the event logs.  This option can be used when
running multiple instances of snort, either on different CPUs, or on same
CPU but different interface.  Each snort instance will use the value
...
The following options can be used:\begin{itemize}\item \texttt{search-method$<$ac$|$ac-std$|$acs$|$\newline ac-banded$|$ac-sparsebands$|$\newline mwm$|$lowmem$>$}
\begin{itemize}
...
\item \texttt{ac-sparebands} Aho-Corasick Sparse-Banded (small memory, high performance)
...
\begin{slist}
\item Faster execution that frag2 with less complex data management.
...
\item Available options:
  \begin{itemize}
   \item     \texttt{max\_frags $<$number$>$} - Maximum simultaneous fragments to track, default 
                          is 8192
                          
   \item     \texttt{memcap $<$bytes$>$} - Memory cap for self preservation, default is 4MB
     
   \item    \texttt{prealloc\_frags $<$number$>$} - alternate memory management mode, use 
...
\hline 
\hline 
\texttt{asynchronous\_link} & Use state transitions based only on one-sided conversation (no tracking of ack/sequence numbers).\\
\hline
\texttt{cache\_clean\_percent} & Purge this percent of least-recently used sessions from the session cache (overrides cache\_clean\_sessions).\\
\hline
\texttt{cache\_clean\_sessions} & Purge this number of least-recently used sessions from the session cache.\\
\hline
...
\hline
\texttt{keepstats} & Records session summary information in $<$logdir$>$/session.log. If no options are specified, output is human readable.\\
\hline
...
\hline
\texttt{memcap~<bytes>} & Number of bytes used to store packets for reassembly.\\
\hline
\texttt{max\_sessions} & Maximum number of simultaneous sessions.\\
\hline
\texttt{noinspect} & Disables stateful inspection.\\
\hline
\texttt{ttl\_limit} & Sets the delta value that will set off an evasion alert.\\
\hline
\texttt{self\_preservation\_threshold} & Limit on number of sessions before entering self-preservation mode (only reassemble data on the default ports).\\
\hline
\texttt{self\_preservation\_period} & Length of time (seconds) to remain in self-preservation mode.\\
\hline
\texttt{suspend\_threshold} & Limit on number of sessions before entering suspend mode (no reassembly).\\
\hline
\texttt{suspend\_period} & Length of time (seconds) to remain in suspend mode.\\
\hline
\texttt{server\_inspect\_limit} & Restrict inspection of server traffic to this many bytes until another client request is seen (ie: client packet with data).\\
\hline
\texttt{state\_protection} & Protect self against DoS attacks.\\
\hline
\end{tabular}
...
\hline
\texttt{flush\_behavior~<number>} & Use specified flush behavior.  Number greater than 0 means use old static flush points.  Number equal to 0 means use new larger flush points.  Number less than 0 means use random flush points defined by flush\_base, flush\_seed, and flush\_range. \\
\hline
\texttt{flush\_base~<number>} & Lowest allowed random flushpoint.  The default value is 512 bytes.  Only used if flush\_behavior is less than 0. \\
\hline
\texttt{flush\_range~<number>} & Space within random flushpoints are generated.  The default value is 1213.  Only used if flush\_behavior is less than 0. \\
\hline
\texttt{flush\_seed~<number>} & Random seed for flushpoints.  The default value is computed from Snort PID + time.  Only used if flush\_behavior is less than 0. \\
\hline
\texttt{overlap\_limit} & Alert when the number of overlapping data bytes reaches a threshold.\\
\hline
...
\hline
max sessions (\texttt{max\_sessions}) & 8192\\
\hline
session cache purge (\texttt{cache\_clean\_sessions}) & 5\\
\hline
session cache purge percent (\texttt{cache\_clean\_percent}) & inactive\\
\hline
...
\begin{note}
The telnet\_decode preprocessor is being deprecated in the next
release in favor of the FTP/Telnet Preprocessor (see Section
\ref{sub:ftptelnet}).
\end{note}
...
\item Time Stamp
\item Drop Rate
\item Mbits/Sec (wire) [Duplicated below to for easy comparison with other rates]
\item Alerts/Sec 
\item K-Pkts/Sec (wire) [Duplicated below to for easy comparison with other rates]
\item Avg Bytes/Pkt (wire) [Duplicated below to for easy comparison with other rates]
...
SMTP has the usual configuration items, such as port and inspection\_type.
...
cmds just checks commands listed with the "normalize\_cmds" parameter.
...
Overrides max\_command\_line\_len for specific commands
...
HTTPInspect (See \ref{sub:http-inspect}).  Users can configure
...
\begin{note}
Some configuration options have an argument of `yes' or `no'.  This
argument specifies whether the user wants the configuration option to
...
\begin{note}
When inspsection\_type is set to stateless, checks for encrypted
traffic will occur on every packet, whereas in stateful mode, 
a particular session will be noted as encrypted and not inspected
any further.
\end{note}
...
\bf{NO INSPECTION} other than TCP state will be performed on FTP
data transfers.  It can be used to improve performance, especially with
large file transfers from a trusted source.   If your ruleset includes
...
be loaded via directives in snort.conf or via command-line options.
...
\hline
\texttt{dynamicengine} & \texttt{dynamicengine $<$shared library path$>$} & Tells snort to load the dynamic engine shared library.  Specify the full or relative path to the shared library.  (Same effect as \texttt{--dynamic-engine-lib} option).  See chapter \ref{Dynamic Modules} for more information on dynamic engine libraries.\\
\hline
...
ruleset, SID 109.  It is implemented to work with the detection engine
provided with snort.
